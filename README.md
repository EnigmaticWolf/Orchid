...
```
## Роутинг
Роутинг запросов происходит по HTTP методу в паре с URL-правилом, каждое правило должено быть отдельно объявленно вызовом метода:
```php
$app->get("/", function() {
...
```
Простой доступ к элементам нового массива с помощью разделителя `/`.
```php
$value = $app["config.foo/bar"]; // вернёт 123
```
## Пути
Сделайте короткие ссылки для файлов/каталогов чтобы получить быстрый доступ к ним:
```php
...
$app->service("db", function(){
    // объект будет создан после первого доступа к $app["db"]
    $obj = new PDO(...);
...
```
Кроме того можно использовать три системные имени для задач:
...
```
## Расширения
При необходимости можно расширить функционал `Orchid` используя расширения:
```php
